...
import { PencilSquareIcon, PlusIcon, FolderOpenIcon, ArrowLeftIcon } from '@heroicons/react/24/outline'
...
export default function ChantierMetresPlanPage() {
  ...
  const loadProjets = async () => {
    try {
      setLoading(true)
      setError(null)
      const response = await fetch(`/api/metres-plan?chantierId=${chantierId}`)
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/metres-plan', {
        ...
        body: JSON.stringify({ nom: nomProjet.trim(), chantierId }),
      })
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50/20 to-indigo-50/10 dark:from-gray-900 dark:via-gray-900 dark:to-gray-900">
      <PageHeader
        ...
        subtitle={chantier ? `Chantier : ${chantier.nomChantier}` : 'Chargement...'}
        ...
        {loading ? (
          ...
          <div className="text-center py-20">
            ...
            <h3 className="text-xl font-semibold text-gray-900 dark:text-white mb-2">
              Aucun métré sur plan pour ce chantier
            </h3>
            <p className="text-gray-500 dark:text-gray-400 mb-6">
              Créez un nouveau métré pour commencer à dessiner sur vos plans PDF.
            </p>
            <button
              onClick={() => setDialogOpen(true)}
      ...
      {dialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm px-4">
          <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md p-6">
            <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-1">Nouveau métré sur plan</h2>
            {chantier && (
              <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">
                Chantier : <span className="font-medium text-gray-700 dark:text-gray-300">{chantier.nomChantier}</span>
              </p>
            )}
            <div>
              ...
              <input
                ...
                placeholder="Ex : Façade - Niveau R+1"
            ...
            <div className="flex justify-end gap-3 mt-6">
              <button
                onClick={() => { setDialogOpen(false); setNomProjet('') }}
