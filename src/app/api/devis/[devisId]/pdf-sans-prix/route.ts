...
import { generateDevisHTMLWithoutPrices } from '@/lib/pdf/templates/devis-template'
...
export async function GET(
  ...
  try {
    ...
    // Générer le HTML sans prix
    const html = generateDevisHTMLWithoutPrices(devisData)
    ...
    const response = new NextResponse(pdfBuffer, {
      headers: {
        ...
        'Content-Disposition': `inline; filename="Devis_${devis.numeroDevis}_sans_prix.pdf"`,
    ...
    console.error('Erreur lors de la génération du PDF sans prix:', error)
    return NextResponse.json(
      { error: 'Erreur lors de la génération du PDF sans prix' },
      { status: 500 }
    )
  }
}
