...
export function ChantierHeader({ chantierId, chantier }: ChantierHeaderProps) {
  ...
  return (
    <div className="relative bg-gradient-to-br from-gray-50 via-blue-50/20 to-purple-50/10 dark:from-gray-900 dark:via-blue-900/5 dark:to-purple-900/5">
      {/* Effet de fond animé */}
      ...
      <div className="relative mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="relative bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border-2 border-white/50 dark:border-gray-700/50 rounded-3xl p-5 shadow-2xl hover:shadow-3xl transition-all duration-500 hover:-translate-y-1">
          
          <div className="relative flex flex-col lg:flex-row items-center gap-4">
            {/* Titre du chantier */}
            <div className="flex-shrink-0 lg:absolute lg:left-4">
              <h1 className={`${getTitleSize(chantier.nomChantier)} font-black text-blue-600 dark:text-blue-400 break-words text-center lg:text-left`}>
            ...
            <nav className="flex flex-wrap gap-3 justify-center flex-1 lg:mx-auto">
            {actions.map((action) => {
              ...
              return (
                <Link
                  key={action.href}
                  href={action.href}
                  className={`group relative flex flex-col items-center justify-center min-w-[80px] px-4 py-3 rounded-xl transition-all duration-300 transform hover:-translate-y-1
                    ${action.isActive 
                ...
                >
                  ...
                  {/* Badge actif - discret */}
                  {action.isActive && (
                    <div className="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 bg-green-500 rounded-full border-2 border-white dark:border-gray-900 shadow-md z-20" />
                  )}
                  
                  <div className="relative z-10 flex flex-col items-center gap-1.5">
                    <div className={`p-2 rounded-lg transition-all duration-300 ${action.isActive ? 'bg-white/20 shadow-inner' : 'bg-gray-100 dark:bg-gray-700/50'}`}>
                      <Icon className={`h-5 w-5 transition-all duration-300 ${action.isActive ? 'text-white drop-shadow-lg' : 'text-gray-600 dark:text-gray-300'}`} />
                    </div>
                    <span className={`text-[10px] font-bold transition-colors leading-tight ${action.isActive ? 'text-white' : 'text-gray-700 dark:text-gray-300'}`}>
                      {action.label}
                    </span>
                  </div>
                </Link>
